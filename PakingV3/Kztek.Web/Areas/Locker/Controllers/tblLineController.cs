...
namespace Kztek.Web.Areas.Locker.Controllers
{
    public class tblLineController : Controller
    {
        ...
        public tblLineController(ItblLockerLineService _tblLockerLineService, ItblLockerPCService _tblLockerPCService)
        {
        ...
        public ActionResult Index(string key = "", string pc = "", int page = 1)
        {
            ...
            var list = _tblLockerLineService.GetAllPagingByFirst(key, pc, page, pageSize);

            var gridModel = PageModelCustom<tblLockerLine>.GetPage(list, page, pageSize);
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Create(tblLockerLine obj, bool SaveAndCountinue = false)
        {
            ViewBag.urlValue = url ?? Request.Url.PathAndQuery;
            ...
            //Thực hiện thêm mới
            var result = _tblLockerLineService.Create(obj);
            if (result.isSuccess)
            {
                WriteLog.Write(result, GetCurrentUser.GetUser(), obj.Id.ToString(), obj.LineName, "tblLockerLine", ConstField.LockerCode, ActionConfigO.Create);
        ...
        public ActionResult Update(string id)
        {
            ...
            var obj = _tblLockerLineService.GetById(id);
            return View(obj);
        }
        ...
        {
            var report = _tblLockerLineService.DeleteById(id);
            if (report.isSuccess)
            {
                WriteLog.Write(report, GetCurrentUser.GetUser(), id, id, "tblLockerLine", ConstField.LockerCode, ActionConfigO.Delete);
            }
        ...
        /// </summary>
        /// <returns></returns>
        private List<tblLockerPC> GetPCList()
        {
            return _tblLockerPCService.GetAll().ToList();
        }
