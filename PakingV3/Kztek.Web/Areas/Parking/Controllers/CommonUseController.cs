...
namespace Kztek.Web.Areas.Parking.Controllers
{
    public class CommonUseController : Controller
    {
        ...
        public PartialViewResult DropdownlistMutilChosenTemplate (DataTable _dt, string iValue, string iText,string itemSecondText, string selectValues,string Modelname)
        {
            ViewBag.Selected = selectValues; // giá trị được chọn
            ViewBag.ivalue = iValue; // cột giá trị lấy theo ID
            ViewBag.itext = iText;// cột hiện thị thứ nhất
            ViewBag.isecondetext = itemSecondText; // cột hiện thị thứ 2
            ViewBag.iName = Modelname; // truyền giá trị từ view theo name bằng jquery
            return PartialView(_dt);
