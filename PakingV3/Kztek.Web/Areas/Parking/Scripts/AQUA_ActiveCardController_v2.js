$(function () {
    $("body").on("change", "input[name=datenewextendfrompicker]", function () {
        ...
        $("#priceUnCheck").val(0);
        $("#checkExtend").val("0");
        AQUA_ActiveCardController_v2.loadData(false);
        AQUA_ActiveCardController_v2.LoadListExtendCard();
    })
    ...
    $("body").on("click", "button[name=btnPrintBill]", function () {
        ...
        if (count === "0") {
            ...
            AQUA_ActiveCardController_v2.GetListCardNumberExtendAll();
        }
        }
...
var AQUA_ActiveCardController_v2 = {
    init: function () {
    ...
    loadData: function (changePageSize) {
        ...
        $.ajax({
            url: _prefixParkingDomain + '/AQUA_ActiveCard/AQUA_boxCards',
            type: 'GET',
            data: {
                key: key,
            ...
            success: function (response) {
                ...
                AQUA_ActiveCardController_v2.BoxMoneyByCardGroup();

                if (totalPage !== "0") {
                    AQUA_ActiveCardController_v2.paging(totalPage, changePageSize);
                    //toastr.success("Tìm kiếm thành công");
                } else {
                    $("#boxCards #pageIndex").text("0");
                    AQUA_ActiveCardController_v2.pagingEmpty();
                }
            }
        });
    },
    ...
    LoadListExtendCard: function () {
        $.ajax({
            ...
            success: function (response) {
                ...
                AQUA_ActiveCardController_v2.GetTotalPriceBoxCardChoice();
            }
        })
    },
    ...
    ExtendAllCard: function () {
        ...
        $.ajax({
            url: _prefixParkingDomain + '/AQUA_ActiveCard/ExtendAllCard_v2',
            data: { obj: obj },
            type: 'json',
            async: false,
            success: function (data) {
                if (data.isSuccess) {
                    ...
                    AQUA_ActiveCardController_v2.loadData();  
                    $("#checkExtend").val("1");
                } else {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result.isSuccess) {
                    if (confirm("Bạn có muốn in hóa đơn không?")) {
                        var url = '/Parking/AQUA_ActiveCard/PrintBill_v2?cardnumbers=' + data.cardnumbers;
                        window.open(url, '_blank');
                    }
                   
                    toastr.success(data.result.Message);
                    AQUA_ActiveCardController_v2.loadData(true);
                    //PK_AQUA_ActiveCardController_v2.LoadListExtendCard();
    ...
    GetListCardNumberExtendAll: function () {
        ...
        $.ajax({
            ...
            //async: true,
            success: function (data) {
                var url = '/Parking/AQUA_ActiveCard/PrintBill_v2?cardnumbers=' + data + '&type=' + '1';
                window.open(url, '_blank');
            }
        });
    },
...
function addExtendCard(id, price) {
    AQUA_ActiveCardController_v2.addListExtendCard(id, price);
}

function UnCheckExtendCard(id,price) {
    AQUA_ActiveCardController_v2.UnCheckExtendCard(id,price);
}

function RemoveAllExtendCard() {
    AQUA_ActiveCardController_v2.removeAllExtend();
}

function RemoveExtendCard(id) {
    AQUA_ActiveCardController_v2.removeExtend(id);
}
...
function ExtendAllCard() {
    ...
    var check = AQUA_ActiveCardController_v2.CheckFeeCardGroup();
    if (check) {
        var count = 0;
        ...
        if (count > 0) {
            if (confirm(confirmExtendCard)) {
                AQUA_ActiveCardController_v2.ExtendAllCard();               
            }
        } else {
...
function ExtendCards() {
    ...
    if (check) {
        ...
        if (number > 0) {
            if (confirm(confirmExtendCard)) {
                AQUA_ActiveCardController_v2.ExtendCards();
            }
        } else {
            toastr.error(messErrExtendCard);
        }
