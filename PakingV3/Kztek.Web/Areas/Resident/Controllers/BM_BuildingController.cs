...
{
    public class BM_BuildingController : Controller
    {
        ...
        //Excell
        //private void PK_CustomerMapFormatCell(List<BM_Building_Excel> listData, string filename = "", string sheetname = "", string comments = "", string titleSheet = "", string titleTime = "")
        //{
        //    //Nội dung đầu trang
        ...
        [HttpPost]
        public ActionResult Create(BM_Building obj, bool SaveAndCountinue = false, string key = "")
        {
            ViewBag.keyValue = key;
            ...
            //Thực hiện thêm mới
            var result = _BM_BuildingService.Create(obj);
            if (result.isSuccess)
            {
                WriteLog.Write(result, GetCurrentUser.GetUser(), obj.Id.ToString(), obj.Name, "BM_Building", ConstField.ResidentCode, ActionConfigO.Create);
        ...
        public ActionResult Update(string id, string key = "", string residentGroup = "", int page = 1)
        {
            ...
            var obj = _BM_BuildingService.GetById(id);

            return View(obj);
        }

        [HttpPost]
        public ActionResult Update(BM_Building obj, string key = "", int page = 1)
        {
            ViewBag.keyValue = key;
            ViewBag.PN = page;

            //Kiểm tra
            var oldObj = _BM_BuildingService.GetById(obj.Id);
            if (oldObj == null)
            {
            ...
            oldObj.FloorNumber = obj.FloorNumber;

            //Thực hiện cập nhật
            var result = _BM_BuildingService.Update(oldObj);
            if (result.isSuccess)
            {
                WriteLog.Write(result, GetCurrentUser.GetUser(), obj.Id.ToString(), obj.Name ,"BM_Building", ConstField.ResidentCode, ActionConfigO.Update);
        ...
        {
            var obj = new BM_Building();

            var result = _BM_BuildingService.DeleteById(id, ref obj);
            if (result.isSuccess)
            {
                WriteLog.Write(result, GetCurrentUser.GetUser(), obj.Id.ToString(), obj.Name, "BM_Building", ConstField.ResidentCode, ActionConfigO.Delete);
            }
